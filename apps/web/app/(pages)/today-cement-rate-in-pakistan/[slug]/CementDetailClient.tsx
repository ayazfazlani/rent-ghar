...
const CONTACT_NUMBER = '923052736792'; // WhatsApp format (no +)
...
export default function CementDetailClient({ rate }: { rate: CementRate }) {
  ...
  const [pageUrl, setPageUrl]     = useState(''); // ← fix: empty string on server
  const inCart  = isInCart(rate._id);
  const images  = getAllImages(rate);

  // ← fix: populate URL only after hydration to avoid SSR mismatch
  useEffect(() => {
  ...
  const shareUrl   = encodeURIComponent(pageUrl); // '' until hydrated — no mismatch
