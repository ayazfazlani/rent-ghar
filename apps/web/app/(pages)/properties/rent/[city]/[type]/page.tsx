...
export async function generateMetadata(
  ...
  if (!cityData) {
    return { title: `Properties for Rent in ${toTitleCase(citySlug)} | Property Dealer` };
  }
  ...
  if (isPropertyType && propertyType) {
    ...
    return {
      ...
      description,
      alternates: {
        canonical: `/properties/rent/${citySlug.toLowerCase()}/${type.toLowerCase()}`,
      },
    };
  }
...
export default async function RentCityTypePage(props: PageProps) {
  ...
  const richDescription = areaData?.description || specificContent?.content || cityData?.rentContent;
